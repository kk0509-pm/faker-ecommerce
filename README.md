...
The repository includes `stress_test_queries.sql` with 25 DuckDB-compatible SQL queries for benchmarking:

- Large result set queries (cross joins, multi-table joins)
- Complex aggregations (GROUP BY, HAVING)
- Window functions (RANK, LAG, running totals)
- CTEs and subqueries
- UNION operations
- Analytical queries (cohort analysis, RFM segmentation)
...
│   └── generators/
│       ├── __init__.py
│       ├── base.py      # Categories, brands, warehouses, coupons
│       ├── customers.py # Customers, addresses
│       ├── products.py  # Products, images, inventory
│       ├── orders.py    # Orders, items, payments, shipments
│       └── reviews.py   # Reviews, wishlists, coupon usage
...
The `setup_db.sh` script helps create a PostgreSQL database and user:

```bash
# Local database
./setup_db.sh <username> <password> <database>

# Remote database (prompts for admin password)
./setup_db.sh <username> <password> <database> <hostname>
```
